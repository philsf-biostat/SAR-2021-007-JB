...
For each APU the incidence rates will be computed as the rate of the number of accidents over
<!-- time. -->
1000 persons.
Incidence will be reported per 1000 persons.

## Covariates

Incidence estimate will be adjusted by Year, and then by Year, all other available covariates.
...
The incidence on each APU will be estimated as the
counts of scorpion accidents
with a Poisson regression with fixed intercepts for each UPA.
The logitudinal effects will be adjusted per year with fixed and random effects for each UPA.
Random slopes for the population and time will be estimated for each UPA.
All model
and random
effects will be estimated without global intercepts.
